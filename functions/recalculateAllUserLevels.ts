...
Deno.serve(async (req) => {
  try {
    ...
    // Level thresholds (updated to 10x)
    ...
    for (const balance of allBalances) {
      ...
      const totalPoints = (balance.total_earned || 0) + (qualityCount * 100);
      ...
      if (balance.total_earned >= 100000) badges.push('coin_collector');
    ...
    return Response.json({ 
      success: true,
      message: `Đã cập nhật level cho ${updatedUsers.length} users`,
      users: updatedUsers
    });
  } catch (error) {
