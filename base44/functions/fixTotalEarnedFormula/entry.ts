...
/**
 * SỬA LẠI TOTAL_EARNED CHO TẤT CẢ USERS THEO CÔNG THỨC MỚI
 * 
 * Công thức ĐÚNG: total_earned = paid_amount + available_balance + admin_review_pending + frozen_balance
 */
...
Deno.serve(async (req) => {
  try {
    ...
    for (const balance of allBalances) {
      ...
      const correctTotal = paid + available + adminReview + frozen;
    ...
    await base44.asServiceRole.entities.CamlycoinTransaction.create({
      ...
      description: `🔧 Sửa total_earned cho ${fixed} users theo công thức: paid + available + admin_review + frozen\n📊 Tổng chênh lệch: ${totalDifference.toLocaleString()}`,
    ...
    return Response.json({
      ...
      formula: 'total_earned = paid_amount + available_balance + admin_review_pending + frozen_balance',
